...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-[100rem] flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent",
        visible &&
          "bg-white/90 dark:bg-neutral-950/80 border border-red-200 shadow-lg",
        className
      )}
    >
      {children}
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-red-50 dark:bg-red-900"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX
      className="text-red-700 dark:text-white cursor-pointer hover:text-red-900"
      onClick={onClick}
    />
  ) : (
    <IconMenu2
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    secondary:
      "bg-transparent text-red-700 hover:text-red-900 hover:bg-red-50 border border-red-200",
    dark: "bg-black text-white shadow-lg",
    gradient: "bg-gradient-to-b from-red-500 to-red-700 text-white shadow-lg",
  };
